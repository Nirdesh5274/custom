      ...
      {items.map((item) => {
        ...
        if (isSimple && item.href) {
          return (
            <Link
              ...
              href={item.href}
              onClick={onNavigate}
              className={`border-b-2 px-2 py-2 text-sm font-extrabold transition ${
                pathname === item.href
        ...
        return (
          <div key={item.label} className="group relative">
            <button
              type="button"
              className="border-b-2 border-transparent px-2 py-2 text-sm font-extrabold text-[#1f3550] transition group-hover:border-[#7ea7d7] group-hover:text-[#295b8f]"
            >
