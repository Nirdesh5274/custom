...
export default async function EventsPage({
  ...
  const events = await prisma.updateItem.findMany({
    where: {
      type: "EVENT",
      isPublished: true,
  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-4 py-10 md:px-6">
      <div className="gov-section p-6 md:p-8">
        <p className="gov-kicker">Outreach and Training Calendar</p>
        <h2 className="mt-2 font-heading text-3xl font-black text-[#0c2f50]">Events and Celebrations</h2>
        <p className="mt-2 text-[#4d627a]">Training, outreach, and ceremonial events conducted by the zone.</p>
        <form className="mt-4 grid gap-3 rounded-xl border border-[#d4dfeb] bg-white p-3 md:grid-cols-[1fr_210px_auto]">
          <input name="q" defaultValue={q} placeholder="Search events" className="input-control" />
          <input name="from" type="date" defaultValue={params.from ?? ""} className="input-control" />
          <button type="submit" className="btn-secondary">Filter</button>
        </form>
        <div className="mt-6">
          <UpdateGrid items={events} emptyMessage="No events published yet." />
        </div>
      </div>
    </section>
  );
